...
from transfermarkt_loader import load_transfermarkt_data, merge_with_player_data
from neural_network import train_neural_network_models, NeuralNetwork
...
def prepare_data():
    """Load and prepare data from CSV files and Transfermarkt data"""
    ...
    # Load CSVs
    print("Loading player statistics data...")
    ...
    # Load Transfermarkt data for real market values
    print("Loading Transfermarkt market value data...")
    transfermarkt_df = load_transfermarkt_data()
    
    # Merge with Transfermarkt data to get real market values
    if not transfermarkt_df.empty:
        df = merge_with_player_data(df, transfermarkt_df)
    
    # Prepare features and targets
    features = []
    performance_targets = []  # For next match performance (goals + assists)
    value_targets = []  # For market value estimation
    real_value_count = 0  # Track how many real market values we use
    synthetic_value_count = 0  # Track how many synthetic values we use
    ...
    for idx, row in df.iterrows():
        ...
        # Use real market value if available from Transfermarkt, otherwise estimate
        real_market_value = row.get('market_value', None)
        if pd.notna(real_market_value) and real_market_value > 0:
            # Use real market value from Transfermarkt
            estimated_value = float(real_market_value)
            real_value_count += 1
        else:
            # Estimated market value in millions (fallback)
            estimated_value = (base_value * age_value_factor * consistency_factor) / 10
            estimated_value = max(0.1, estimated_value)
            synthetic_value_count += 1
        
        value_targets.append(estimated_value)
    
    print(f"\nMarket Value Data Statistics:")
    print(f"  - Real market values from Transfermarkt: {real_value_count}")
    print(f"  - Synthetic/estimated market values: {synthetic_value_count}")
    print(f"  - Total samples: {len(value_targets)}")
    if len(value_targets) > 0:
        print(f"  - Real value coverage: {real_value_count/len(value_targets)*100:.1f}%")
    
    return np.array(features), np.array(performance_targets), np.array(value_targets)

def train_models(use_neural_network=True, use_random_forest=True):
    """
    Train ML models for performance and value prediction
    
    Args:
        use_neural_network: If True, train neural network models (default: True)
        use_random_forest: If True, train random forest models (default: True)
    
    Returns:
        Tuple of (perf_model, value_model, scaler)
        Model type depends on which models were trained
    """
    ...
    print(f"Data split: {len(X_train)} training samples ({len(X_train)/len(X)*100:.1f}%), {len(X_test)} test samples ({len(X_test)/len(X)*100:.1f}%)")
    ...
    if use_random_forest:
        ...
        print("\nTraining performance prediction model (Random Forest)...")
        perf_model_rf = RandomForestRegressor(n_estimators=100, max_depth=10, random_state=42, n_jobs=-1)
        perf_model_rf.fit(X_train_scaled, y_perf_train)
        
        perf_pred = perf_model_rf.predict(X_test_scaled)
        perf_mae = mean_absolute_error(y_perf_test, perf_pred)
        perf_r2 = r2_score(y_perf_test, perf_pred)
        print(f"Performance Model (RF) - MAE: {perf_mae:.3f}, R²: {perf_r2:.3f}")
        
        # Train value prediction model
        print("\nTraining market value prediction model (Random Forest)...")
        value_model_rf = RandomForestRegressor(n_estimators=100, max_depth=10, random_state=42, n_jobs=-1)
        value_model_rf.fit(X_train_scaled, y_value_train)
        
        value_pred = value_model_rf.predict(X_test_scaled)
        value_mae = mean_absolute_error(y_value_test, value_pred)
        value_r2 = r2_score(y_value_test, value_pred)
        print(f"Value Model (RF) - MAE: ${value_mae:.2f}M, R²: {value_r2:.3f}")
        
        # Save Random Forest models
        with open(os.path.join(models_dir, "perf_model.pkl"), "wb") as f:
            pickle.dump(perf_model_rf, f)
        
        with open(os.path.join(models_dir, "value_model.pkl"), "wb") as f:
            pickle.dump(value_model_rf, f)
        
        perf_model = perf_model_rf
        value_model = value_model_rf
    
    # Train Neural Network models
    if use_neural_network:
        perf_model_nn, value_model_nn = train_neural_network_models(
            X_train, y_perf_train, y_value_train,
            X_test, y_perf_test, y_value_test,
            scaler, models_dir, epochs=100, batch_size=32
        )
        
        # Use neural network models if they were trained
        if use_neural_network and not use_random_forest:
            perf_model = perf_model_nn
            value_model = value_model_nn
